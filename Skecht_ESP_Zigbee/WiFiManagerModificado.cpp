/**
 * WiFiManagerModificado.cpp
 * 
 * WiFiManagerModificado, a library for the ESP8266/Arduino platform
...
#ifdef ESP32
uint8_t WiFiManagerModificado::_lastconxresulttmp = WL_IDLE_STATUS;
#endif

/**
 * --------------------------------------------------------------------------------
 *  WiFiManagerModificadoParameter
 * --------------------------------------------------------------------------------
**/

WiFiManagerModificadoParameter::WiFiManagerModificadoParameter() {
  WiFiManagerModificadoParameter("");
}

WiFiManagerModificadoParameter::WiFiManagerModificadoParameter(const char *custom) {
  _id             = NULL;
  _label          = NULL;
  _length         = 1;
  _value          = NULL;
...
WiFiManagerModificadoParameter::WiFiManagerModificadoParameter(const char *id, const char *label) {
  init(id, label, "", 0, "", WFM_LABEL_BEFORE);
}

WiFiManagerModificadoParameter::WiFiManagerModificadoParameter(const char *id, const char *label, const char *defaultValue, int length) {
  init(id, label, defaultValue, length, "", WFM_LABEL_BEFORE);
}

WiFiManagerModificadoParameter::WiFiManagerModificadoParameter(const char *id, const char *label, const char *defaultValue, int length, const char *custom) {
  init(id, label, defaultValue, length, custom, WFM_LABEL_BEFORE);
}

WiFiManagerModificadoParameter::WiFiManagerModificadoParameter(const char *id, const char *label, const char *defaultValue, int length, const char *custom, int labelPlacement) {
  init(id, label, defaultValue, length, custom, labelPlacement);
}

void WiFiManagerModificadoParameter::init(const char *id, const char *label, const char *defaultValue, int length, const char *custom, int labelPlacement) {
  _id             = id;
  _label          = label;
...
WiFiManagerModificadoParameter::~WiFiManagerModificadoParameter() {
  ...
  _length=0; // setting length 0, ideally the entire parameter should be removed, or added to WiFiManagerModificado scope so it follows
}

// WiFiManagerModificadoParameter& WiFiManagerModificadoParameter::operator=(const WiFiManagerModificadoParameter& rhs){
...
void WiFiManagerModificadoParameter::setValue(const char *defaultValue, int length) {
  if(!_id){
...
const char* WiFiManagerModificadoParameter::getValue() {
  // Serial.println(printf("Address of _value is %p\n", (void *)_value)); 
  return _value;
}
const char* WiFiManagerModificadoParameter::getID() {
  return _id;
}
const char* WiFiManagerModificadoParameter::getPlaceholder() {
  return _label;
}
const char* WiFiManagerModificadoParameter::getLabel() {
  return _label;
}
int WiFiManagerModificadoParameter::getValueLength() {
  return _length;
}
int WiFiManagerModificadoParameter::getLabelPlacement() {
  return _labelPlacement;
}
const char* WiFiManagerModificadoParameter::getCustomHTML() {
  return _customHTML;
}
 ...
 * @access public
 * @param {[type]} WiFiManagerModificadoParameter *p [description]
 */
bool WiFiManagerModificado::addParameter(WiFiManagerModificadoParameter *p) {
  ...
  if(_params == NULL){
    DEBUG_WM(DEBUG_DEV,"allocating params bytes:",_max_params * sizeof(WiFiManagerModificadoParameter*));        
    _params = (WiFiManagerModificadoParameter**)malloc(_max_params * sizeof(WiFiManagerModificadoParameter*));
  }
  ...
  if(_paramsCount == _max_params){
    ...
    DEBUG_WM(DEBUG_DEV,"re-allocating params bytes:",_max_params * sizeof(WiFiManagerModificadoParameter*));    
    WiFiManagerModificadoParameter** new_params = (WiFiManagerModificadoParameter**)realloc(_params, _max_params * sizeof(WiFiManagerModificadoParameter*));
 ...
 * @access public
 */
WiFiManagerModificadoParameter** WiFiManagerModificado::getParameters() {
  return _params;
}
 ...
 * @access public
 */
int WiFiManagerModificado::getParametersCount() {
  return _paramsCount;
}

/**
 * --------------------------------------------------------------------------------
 *  WiFiManagerModificado 
 * --------------------------------------------------------------------------------
**/

// constructors
WiFiManagerModificado::WiFiManagerModificado(Stream& consolePort):_debugPort(consolePort){
  WiFiManagerModificadoInit();
}

WiFiManagerModificado::WiFiManagerModificado() {
  WiFiManagerModificadoInit();  
}

void WiFiManagerModificado::WiFiManagerModificadoInit(){
...
WiFiManagerModificado::~WiFiManagerModificado() {
  _end();
  // parameters
  // @todo below belongs to WiFiManagerModificadoparameter
  if (_params != NULL){
  ...
  // @todo remove event
  // WiFi.onEvent(std::bind(&WiFiManagerModificado::WiFiEvent,this,_1,_2));
  #ifdef ESP32
...
void WiFiManagerModificado::_begin(){
  if(_hasBegun) return;
  _hasBegun = true;
...
void WiFiManagerModificado::_end(){
  _hasBegun = false;
...
boolean WiFiManagerModificado::autoConnect() {
 ...
 */
boolean WiFiManagerModificado::autoConnect(char const *apName, char const *apPassword) {
...
bool WiFiManagerModificado::setupHostname(bool restart){
  if((String)_hostname == "") {
...
// CONFIG PORTAL
bool WiFiManagerModificado::startAP(){
  bool ret = true;
 ...
 * @return {[type]} [description]
 */
void WiFiManagerModificado::startWebPortal() {
 ...
 * @return {[type]} [description]
 */
void WiFiManagerModificado::stopWebPortal() {
  if(!configPortalActive && !webPortalActive) return;
...
boolean WiFiManagerModificado::configPortalHasTimeout(){
...
void WiFiManagerModificado::setupConfigPortal() {
  ...
  server->on(String(FPSTR(R_root)).c_str(),       std::bind(&WiFiManagerModificado::handleRoot, this));
  server->on(String(FPSTR(R_wifi)).c_str(),       std::bind(&WiFiManagerModificado::handleWifi, this, true));
  server->on(String(FPSTR(R_wifinoscan)).c_str(), std::bind(&WiFiManagerModificado::handleWifi, this, false));
  server->on(String(FPSTR(R_wifisave)).c_str(),   std::bind(&WiFiManagerModificado::handleWifiSave, this));
  server->on(String(FPSTR(R_info)).c_str(),       std::bind(&WiFiManagerModificado::handleInfo, this));
  server->on(String(FPSTR(R_param)).c_str(),      std::bind(&WiFiManagerModificado::handleParam, this));
  server->on(String(FPSTR(R_paramsave)).c_str(),  std::bind(&WiFiManagerModificado::handleParamSave, this));
  server->on(String(FPSTR(R_restart)).c_str(),    std::bind(&WiFiManagerModificado::handleReset, this));
  server->on(String(FPSTR(R_exit)).c_str(),       std::bind(&WiFiManagerModificado::handleExit, this));
  server->on(String(FPSTR(R_close)).c_str(),      std::bind(&WiFiManagerModificado::handleClose, this));
  server->on(String(FPSTR(R_erase)).c_str(),      std::bind(&WiFiManagerModificado::handleErase, this, false));
  server->on(String(FPSTR(R_status)).c_str(),     std::bind(&WiFiManagerModificado::handleWiFiStatus, this));
  server->onNotFound (std::bind(&WiFiManagerModificado::handleNotFound, this));
...
boolean WiFiManagerModificado::startConfigPortal() {
  String ssid = getDefaultAPName();
 ...
 * @return {[type]}      [description]
 */
boolean  WiFiManagerModificado::startConfigPortal(char const *apName, char const *apPassword) {
  _begin();
 ...
 * @return {[type]} [description]
 */
boolean WiFiManagerModificado::process(){
...
uint8_t WiFiManagerModificado::processConfigPortal(){
    //DNS handler
 ...
 */
bool WiFiManagerModificado::shutdownConfigPortal(){
...
uint8_t WiFiManagerModificado::connectWifi(String ssid, String pass) {
 ...
 * @return bool success
 */
bool WiFiManagerModificado::wifiConnectNew(String ssid, String pass){
  bool ret = false;
 ...
 * @return bool success
 */
bool WiFiManagerModificado::wifiConnectDefault(){
  bool ret = false;
 ...
 * @return bool success
 */
bool WiFiManagerModificado::setSTAConfig(){
...
void WiFiManagerModificado::updateConxResult(uint8_t status){
...
uint8_t WiFiManagerModificado::waitForConnectResult() {
 ...
 */
uint8_t WiFiManagerModificado::waitForConnectResult(uint16_t timeout) {
  if (timeout == 0){
...
// WPS enabled? https://github.com/esp8266/Arduino/pull/4889
#ifdef NO_EXTRA_4K_HEAP
void WiFiManagerModificado::startWPS() {
...
String WiFiManagerModificado::getHTTPHead(String title){
  String page;
 ...
 */
void WiFiManagerModificado::handleRequest() {
 ...
 */
void WiFiManagerModificado::handleRoot() {
 ...
 */
void WiFiManagerModificado::handleWifi(boolean scan) {
 ...
 */
void WiFiManagerModificado::handleParam(){
...
String WiFiManagerModificado::getMenuOut(){
  String page;  
...
// bool WiFiManagerModificado::WiFi_scanNetworksForAP(bool force){
//   WiFi_scanNetworks(force);
// }

void WiFiManagerModificado::WiFi_scanComplete(int networksFound){
  _lastscan = millis();
...
bool WiFiManagerModificado::WiFi_scanNetworks(){
  return WiFi_scanNetworks(false,false);
}

bool WiFiManagerModificado::WiFi_scanNetworks(unsigned int cachetime,bool async){
    return WiFi_scanNetworks(millis()-_lastscan > cachetime,async);
}
bool WiFiManagerModificado::WiFi_scanNetworks(unsigned int cachetime){
    return WiFi_scanNetworks(millis()-_lastscan > cachetime,false);
}
bool WiFiManagerModificado::WiFi_scanNetworks(bool force,bool async){
    ...
    if(force || _numNetworks == 0 || (millis()-_lastscan > 60000)){
      ...
      if(async && _asyncScan){
          ...
          WiFi.scanNetworksAsync(std::bind(&WiFiManagerModificado::WiFi_scanComplete,this,_1));
          #else
...
String WiFiManagerModificado::WiFiManagerModificado::getScanItemOut(){
    String page;
...
String WiFiManagerModificado::getIpForm(String id, String title, String value){
...
String WiFiManagerModificado::getStaticOut(){
  String page;
...
String WiFiManagerModificado::getParamOut(){
  String page;
  ...
  if(_paramsCount > 0){
    ...
    for (int i = 0; i < _paramsCount; i++) {
      if (_params[i] == NULL || _params[i]->_length == 0) {
        DEBUG_WM(DEBUG_ERROR,"[ERROR] WiFiManagerModificadoParameter is out of scope");
        break;
      }
...
void WiFiManagerModificado::handleWiFiStatus(){
 ...
 */
void WiFiManagerModificado::handleWifiSave() {
...
void WiFiManagerModificado::handleParamSave() {
...
void WiFiManagerModificado::doParamSave(){
 ...
 */
void WiFiManagerModificado::handleInfo() {
...
String WiFiManagerModificado::getInfoData(String id){
 ...
 * HTTPD CALLBACK root or redirect to captive portal
 */
void WiFiManagerModificado::handleExit() {
 ...
 */
void WiFiManagerModificado::handleReset() {
...
// void WiFiManagerModificado::handleErase() {
//   handleErase(false);
// }
void WiFiManagerModificado::handleErase(boolean opt) {
 ...
 * HTTPD CALLBACK 404
 */
void WiFiManagerModificado::handleNotFound() {
  if (captivePortal()) return; // If captive portal redirect instead of displaying the page
  handleRequest();
 ...
 */
boolean WiFiManagerModificado::captivePortal() {
...
void WiFiManagerModificado::reportStatus(String &page){
 ...
 * @return {[type]} [description]
 */
bool WiFiManagerModificado::stopConfigPortal(){
  if(_configPortalIsBlocking){
    abort = true;
    return true;
  }
 ...
 * @return bool success
 */
bool WiFiManagerModificado::disconnect(){
  if(WiFi.status() != WL_CONNECTED){
 ...
 * @access public
 */
void WiFiManagerModificado::reboot(){
  DEBUG_WM("Restarting");
  ESP.restart();
}
 ...
 * @access public
 */
bool WiFiManagerModificado::erase(){
  return erase(false);
}

bool WiFiManagerModificado::erase(bool opt){
  DEBUG_WM("Erasing");
 ...
 * @access public
 */
void WiFiManagerModificado::resetSettings() {
 ...
 */
void WiFiManagerModificado::setTimeout(unsigned long seconds) {
 ...
 * @param {[type]} unsigned long seconds [description]
 */
void WiFiManagerModificado::setConfigPortalTimeout(unsigned long seconds) {
 ...
 * @param {[type]} unsigned long seconds [description]
 */
void WiFiManagerModificado::setConnectTimeout(unsigned long seconds) {
 ...
 */
void WiFiManagerModificado::setCleanConnect(bool enable){
 ...
 * @param {[type]} unsigned long seconds [description]
 */
void WiFiManagerModificado::setSaveConnectTimeout(unsigned long seconds) {
 ...
 */
void WiFiManagerModificado::setDebugOutput(boolean debug) {
  _debug = debug;
  if(_debug && _debugLevel == DEBUG_DEV) debugPlatformInfo();
}
 ...
 */
void WiFiManagerModificado::setAPStaticIPConfig(IPAddress ip, IPAddress gw, IPAddress sn) {
  _ap_static_ip = ip;
  _ap_static_gw = gw;
  _ap_static_sn = sn;
}
 ...
 * @param {[type]} IPAddress sn [description]
 */
void WiFiManagerModificado::setSTAStaticIPConfig(IPAddress ip, IPAddress gw, IPAddress sn) {
  _sta_static_ip = ip;
  _sta_static_gw = gw;
  _sta_static_sn = sn;
}
 ...
 */
void WiFiManagerModificado::setSTAStaticIPConfig(IPAddress ip, IPAddress gw, IPAddress sn, IPAddress dns) {
 ...
 */
void WiFiManagerModificado::setMinimumSignalQuality(int quality) {
 ...
 */
void WiFiManagerModificado::setBreakAfterConfig(boolean shouldBreak) {
 ...
 * @access public 
 * @param {[type]} void (*func)(WiFiManagerModificado* wminstance)
 */
void WiFiManagerModificado::setAPCallback( std::function<void(WiFiManagerModificado*)> func ) {
  _apcallback = func;
}
 ...
 */
void WiFiManagerModificado::setWebServerCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void (*func)(void)
 */
void WiFiManagerModificado::setSaveConfigCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void(*func)(void)
 */
void WiFiManagerModificado::setConfigResetCallback( std::function<void()> func ) {
    _resetcallback = func;
}
 ...
 * @param {[type]} void (*func)(void)
 */
void WiFiManagerModificado::setSaveParamsCallback( std::function<void()> func ) {
 ...
 * @param {[type]} void (*func)(void)
 */
void WiFiManagerModificado::setPreSaveConfigCallback( std::function<void()> func ) {
 ...
 * @param char element
 */
void WiFiManagerModificado::setCustomHeadElement(const char* element) {
 ...
 */
void WiFiManagerModificado::setRemoveDuplicateAPs(boolean removeDuplicates) {
 ...
 */
void WiFiManagerModificado::setConfigPortalBlocking(boolean shoudlBlock) {
 ...
 */
void WiFiManagerModificado::setRestorePersistent(boolean persistent) {
 ...
 */
void WiFiManagerModificado::setShowStaticFields(boolean alwaysShow){
 ...
 * @param boolean alwaysShow [false]
 */
void WiFiManagerModificado::setShowDnsFields(boolean alwaysShow){
 ...
 * @param boolean alwaysShow [false]
 */
void WiFiManagerModificado::setShowPassword(boolean show){
  _showPassword = show;
}
 ...
 */
void WiFiManagerModificado::setCaptivePortalEnable(boolean enabled){
 ...
 * @param boolean enabled [true]
 */
void WiFiManagerModificado::setWiFiAutoReconnect(boolean enabled){
 ...
 */
void WiFiManagerModificado::setAPClientCheck(boolean enabled){
 ...
 * @param boolean enabled [true]
 */
void WiFiManagerModificado::setWebPortalClientCheck(boolean enabled){
 ...
 * @param boolean enabled [false]
 */
void WiFiManagerModificado::setScanDispPerc(boolean enabled){
 ...
 * @param boolean enabled [true]
 */
void WiFiManagerModificado::setEnableConfigPortal(boolean enable)
{
 ...
 */
bool  WiFiManagerModificado::setHostname(const char * hostname){
  //@todo max length 32
  _hostname = hostname;
  return true;
}
 ...
 */
void WiFiManagerModificado::setWiFiAPChannel(int32_t channel){
  _apChannel = channel;
}
 ...
 */
void WiFiManagerModificado::setWiFiAPHidden(bool hidden){
  _apHidden = hidden;
}
 ...
 */
void WiFiManagerModificado::setShowInfoErase(boolean enabled){
 ...
 *  WiFiManagerModificado.setMenu(menu);
 * @since $dev
 * @param uint8_t menu[] array of menu ids
 */
void WiFiManagerModificado::setMenu(const char * menu[], uint8_t size){
 ...
 */
void WiFiManagerModificado::setMenu(std::vector<const char *>& menu){
 ...
 * @since $dev
 */
void WiFiManagerModificado::setParamsPage(bool enable){
 ...
 */
String WiFiManagerModificado::getConfigPortalSSID() {
  return _apName;
}
 ...
 */
uint8_t WiFiManagerModificado::getLastConxResult(){
 ...
 */
bool WiFiManagerModificado::getWiFiIsSaved(){
  return WiFi_hasAutoConnect();
}

String WiFiManagerModificado::getDefaultAPName(){
 ...
 */
void WiFiManagerModificado::setCountry(String cc){
  _wificountry = cc;
}
 ...
 */
void WiFiManagerModificado::setClass(String str){
  _bodyClass = str;
}

void WiFiManagerModificado::setHttpPort(uint16_t port){
  _httpPort = port;
}
 ...
 * @return String
 */
String WiFiManagerModificado::getWiFiSSID(bool persistent){
 ...
 * @return String
 */
String WiFiManagerModificado::getWiFiPass(bool persistent){
...
void WiFiManagerModificado::DEBUG_WM(Generic text) {
  DEBUG_WM(DEBUG_NOTIFY,text,"");
}

template <typename Generic>
void WiFiManagerModificado::DEBUG_WM(wm_debuglevel_t level,Generic text) {
  if(_debugLevel >= level) DEBUG_WM(level,text,"");
}

template <typename Generic, typename Genericb>
void WiFiManagerModificado::DEBUG_WM(Generic text,Genericb textb) {
  DEBUG_WM(DEBUG_NOTIFY,text,textb);
}

template <typename Generic, typename Genericb>
void WiFiManagerModificado::DEBUG_WM(wm_debuglevel_t level,Generic text,Genericb textb) {
 ...
 * @return {[type]} [description]
 */
void WiFiManagerModificado::debugSoftAPConfig(){
 ...
 * @return {[type]} [description]
 */
void WiFiManagerModificado::debugPlatformInfo(){
  #ifdef ESP8266
...
int WiFiManagerModificado::getRSSIasQuality(int RSSI) {
  int quality = 0;
...
/** Is this an IP? */
boolean WiFiManagerModificado::isIp(String str) {
...
/** IP to String? */
String WiFiManagerModificado::toStringIp(IPAddress ip) {
  String res = "";
  for (int i = 0; i < 3; i++) {
...
boolean WiFiManagerModificado::validApPassword(){
 ...
 */
String WiFiManagerModificado::htmlEntities(String str) {
  str.replace("&","&amp;");
  str.replace("<","&lt;");
  str.replace(">","&gt;");
  // str.replace("'","&#39;");
 ...
 * @return {[type]}         [description]
 */
String WiFiManagerModificado::getWLStatusString(uint8_t status){
  if(status <= 7) return WIFI_STA_STATUS[status];
  return FPSTR(S_NA);
}

String WiFiManagerModificado::encryptionTypeStr(uint8_t authmode) {
  // DEBUG_WM("enc_tye: ",authmode);
  return AUTH_MODE_NAMES[authmode];
}

String WiFiManagerModificado::getModeString(uint8_t mode){
  if(mode <= 3) return WIFI_MODES[mode];
  return FPSTR(S_NA);
}

bool WiFiManagerModificado::WiFiSetCountry(){
...
bool WiFiManagerModificado::WiFi_Mode(WiFiMode_t m,bool persistent) {
    bool ret;
    #ifdef ESP8266
...
bool WiFiManagerModificado::WiFi_Mode(WiFiMode_t m) {
	return WiFi_Mode(m,false);
}

// sta disconnect without persistent
bool WiFiManagerModificado::WiFi_Disconnect() {
    #ifdef ESP8266
...
bool WiFiManagerModificado::WiFi_enableSTA(bool enable,bool persistent) {
...
bool WiFiManagerModificado::WiFi_enableSTA(bool enable) {
	return WiFi_enableSTA(enable,false);
}

bool WiFiManagerModificado::WiFi_eraseConfig() {
    #ifdef ESP8266
...
uint8_t WiFiManagerModificado::WiFi_softap_num_stations(){
  #ifdef ESP8266
...
bool WiFiManagerModificado::WiFi_hasAutoConnect(){
  return WiFi_SSID(true) != "";
}

String WiFiManagerModificado::WiFi_SSID(bool persistent) const{
...
String WiFiManagerModificado::WiFi_psk(bool persistent) const {
    #ifdef ESP8266
    struct station_config conf;
...
#ifdef ESP32
void WiFiManagerModificado::WiFiEvent(WiFiEvent_t event,system_event_info_t info){
    if(!_hasBegun){
...
void WiFiManagerModificado::WiFi_autoReconnect(){
  #ifdef ESP8266
    ...
    // if(_wifiAutoReconnect){
      ...
      wm_event_id = WiFi.onEvent(std::bind(&WiFiManagerModificado::WiFiEvent,this,_1,_2));
    // }
  #endif
}
